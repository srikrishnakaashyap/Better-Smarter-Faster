...
class Agent1:
    ...
    def computeUtilityAcrossActions(
        ...
        if actions[0] == 0:
            ...
            agentMoves = minimumDistanceMap.get(min(minimumDistanceMap), [])
        else:
            agentNeighbours = Utility.getNeighbours(graph, currState[0])
            maximumDistanceMap = defaultdict(list)
            for n in agentNeighbours:
                maximumDistanceMap[dist[n][currState[2]]].append(n)
            agentMoves = maximumDistanceMap.get(max(maximumDistanceMap), [])
        preyMoves = Utility.getNeighbours(graph, currState[1], include=True)
        if actions[2] == 0:
            predNeighbours = Utility.getNeighbours(graph, currState[2])
            minimumDistanceMap = defaultdict(list)
            for n in predNeighbours:
                minimumDistanceMap[dist[n][currState[0]]].append(n)
            predMoves = minimumDistanceMap.get(min(minimumDistanceMap), [])
        else:
            predMoves = Utility.getNeighbours(graph, currState[2])
        sum = 0
        numberofstates = len(agentMoves) * len(preyMoves) * len(predMoves)
        for agentMove in agentMoves:
            for preyMove in preyMoves:
                for predMove in predMoves:
                    sum += (1 / numberofstates) * utility[agentMove][preyMove][predMove]
        return sum
            ...
            for agent in range(size):
                for prey in range(size):
                    for pred in range(size):
                        ...
                        nextUtility[agent][prey][pred] = self.getReward(
                            (agent, prey, pred), dist
    ...
    def getReward(self, state, dist):
        preyDist = dist[state[0]][state[1]]
        predDistance = dist[state[0]][state[2]]

        return 10 * predDistance / (1 + preyDist)
